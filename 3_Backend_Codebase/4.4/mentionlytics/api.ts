...
// Get the Mentionlytics API token from secrets
...
async function callMentionlyticsAPI(endpoint: string, params?: any) {
  ...
  if (!token) {
    console.error("MENTIONLYTICS_API_TOKEN not configured");
    throw APIError.internal("Mentionlytics API not configured");
  }
