...
// Get notification service credentials from secrets (with fallbacks for deployment)
...
// Helper functions to safely get secrets
async function getTwilioCredentials(): Promise<{ sid: string | null; token: string | null }> {
  try {
    const sid = await twilioAccountSid();
    const token = await twilioAuthToken();
    return { sid: sid || null, token: token || null };
  } catch (error) {
    console.warn("Twilio credentials not configured, SMS alerts will use mock mode");
    return { sid: null, token: null };
  }
}

async function getEmailApiKey(): Promise<string | null> {
  try {
    const key = await emailApiKey();
    return key || null;
  } catch (error) {
    console.warn("EMAIL_API_KEY not configured, email alerts will use mock mode");
    return null;
  }
}
        ...
        setTimeout(async () => {
          ...
          if (alert) {
            ...
            if (type === "sms") {
              ...
              hasCredentials = true; // Push/webhook don't need special credentials
            }
            
            if (hasCredentials) {
              console.log(`Sending LIVE ${type} alert to ${recipient}`);
              // TODO: Implement real sending logic here
            } else {
              console.log(`Mock ${type} alert sent to ${recipient} (credentials not configured)`);
            }
            ...
            if (Math.random() > 0.1) {
              ...
              alert.status = "failed";
              alert.error = hasCredentials ? "Delivery failed" : "Credentials not configured";
            }
          }
