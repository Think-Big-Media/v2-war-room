...
// Get JWT secret from secrets (with fallback)
const jwtSecret = secret("JWT_SECRET");
const JWT_FALLBACK_SECRET = "development-fallback-secret-change-in-production";
...
export const login = api<LoginRequest, LoginResponse>(
  ...
  async ({ username, password }) => {
    ...
    let secret: string;
    try {
      secret = await jwtSecret();
    } catch (error) {
      console.warn("JWT_SECRET not configured, using fallback for development");
      secret = JWT_FALLBACK_SECRET;
    }
    
    if (!secret) {
      console.warn("JWT_SECRET empty, using fallback for development");
      secret = JWT_FALLBACK_SECRET;
    }
